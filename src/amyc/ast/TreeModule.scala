...
trait TreeModule {
  ...
  case class ArrayLiteral(length: Int, value: List[Int]) extends Literal[List[Int]]               //    <===
  ...
  case class Call(qname: QualifiedName, argsAndRangeChecks: List[(Expr, Boolean)]) extends Expr   //    <===
  // Array accessing
  case class ArrayAccess(variable: Variable, elem: Expr, rangeCheck: Boolean) extends Expr        //    <===
  // Length of an array
  case class ArrayLength(variable: Variable) extends Expr                                         //    <===
  // The ; operator
  case class Sequence(e1: Expr, e2: Expr) extends Expr
  // Local variable definition
  case class Let(df: ParamDef, value: Expr, body: Expr, rangeCheck: Boolean) extends Expr         //    <===
  // If-then-else
  ...
  case class RangeType(start: Int, end: Int) extends Type {                                 //    <===
    override def toString: String = {
      if (start < end) {
        "Int from " + start.toString + " to " + end.toString
      } else {
        "Int"
      }
    }
  } 
  case class ArrayType(length: Int) extends Type {                                          //    <===
    override def toString: String = "Array of length " + length.toString
  }
  case object GenericArrayType extends Type {                                               //    <===
